...
我甚至不知道 `getSlots()` 根本不存在，被 VSCode 的 `💣 类型"Skeleton"上不存在属性"getSlots"` 打脸了。

然后我猜底层 skeleton 在 `this.spine.skeleton`，运行时又是 `undefined`。直到翻了 Cocos 引擎源码才发现：

```typescript
// cocos/spine/skeleton.ts
public _skeleton: spine.Skeleton = null!; // 公开字段，只是名字带下划线
```

> **教训**：不确定的 API 先直接调，不要预判性绕路写 intersection type。

### 第二阶段：Slot 附件读不到

`_skeleton.slots` 能拿到所有插槽了，但 `slot.attachment` 全是 `null`。

查了项目里现成的代码才发现——Creator 3.8 的 spine 是 WASM runtime，Slot 的方法要用调用形式：

```typescript
slot.getAttachment() // ✅ 方法
slot.attachment      // ❌ 属性，WASM 下始终 null
```

还有一个坑：`setSkin` 之后必须调 `setSlotsToSetupPose()`，否则 slot 的 attachment 不会被填充。

### 第三阶段：`setSkin` 互相覆盖

上装和下装各有一个按钮，分别调 `setSkin("shangzhuang")` 和 `setSkin("xiazhuang")`。点完上装再点下装，上装没了。

Spine 的 `setSkin` 就是替换语义，不是叠加。正确做法是从 `skeletonData` 中取出各个 skin，用 `addSkin` 合并。

### 第四阶段：旧代码的"全部加载"策略

项目里已有的换装实现将所有时装的纹理都挂载到显存里——12 部位 × 海量装备 = 显存爆炸。

你的原话点醒了我：

> **"你怎么又被以前写得旧代码影响了？以前是错误的范式。"**

正确的做法：**只加载当前 skin 实际激活的那些插槽的纹理**。先 `setSkin` 看哪些插槽有附件，再按需 `setSlotTexture`。

### 第五阶段：过滤本体插槽

换装前后的对比发现——有些插槽（手脚、头脸、眼耳）始终激活，它们是角色本体，不是装扮。

把这部分预采集到 `baseSlots` 里，每次换装做一次集合减法，过滤掉本体噪声。
